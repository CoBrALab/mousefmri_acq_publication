...
df1<-droplevels(subset(DR_df, isoflurane_percent != 1.5)) #this removed 5% of the datapoints
df2<-droplevels(subset(df1, Number.of.Timepoints >= 90)) #this removed another 16% of the datapoints

#drop timepoints from iso 0.5 and 1 to match the number of timepoints at iso 0.23 (106), which has the fewest
set.seed(123)
trim_group <- function(df_group, target, tolerance = 15) {

  n_current <- nrow(df_group)
  max_n <- target + tolerance

  # No trimming needed
  if (n_current <= max_n)
    return(df_group)

  # Eligible rows (only timepoint = 120)
  candidates <- df_group %>%
    mutate(
      row_index = row_number(),
      priority =
        4 * (strain == "C3HeB/FeJ") +
        2 * (actual_ses_order %in% c(3)) +
        2 * (dex_conc %in% c(0.025, 0.1)) + 
        3 * (Time.after.isoflurane.change %in% c(120, 240))
    ) %>%
    filter(Number.of.Timepoints >= 119)

  n_drop_needed <- n_current - target

  drop_rows <- c()

  # Work through priorities highest → lowest
  for (p in sort(unique(candidates$priority), decreasing = TRUE)) {

    tier <- candidates %>% filter(priority == p)

    if (nrow(tier) == 0) next

    # How many to drop from this tier?
    n_to_take <- min(n_drop_needed, nrow(tier))

    # Random sample FROM THIS PRIORITY LEVEL ONLY
    chosen <- sample(tier$row_index, n_to_take)

    drop_rows <- c(drop_rows, chosen)

    n_drop_needed <- n_drop_needed - n_to_take

    if (n_drop_needed <= 0) break
  }

  # Drop rows by row index (safe inside group)
  df_group[-drop_rows, ]
}

# Apply to the full dataset grouped by Iso.percent
df <- df2 %>%
  group_by(Iso.percent) %>%
  group_modify(~ trim_group(.x, target = 106, tolerance = 5)) %>%
  ungroup()
...
sink("./mediation_stats_tables/pathctotal_outputs_noquadratic.txt")
summary(medc) 
sink()  
...
sink("./mediation_stats_tables/pathb_outputs_noquadratic.txt")
summary(medb) 
sink()  
...
pathb_effects_tibble$Difference <- list('0.01', '0.00', '0.02', '0.00', '-0.01', '0.01', '0.00', '0.00')

pathb_effects_table <- gt(pathb_effects_tibble %>% 
                            dplyr::select(`Predictors`, Estimate, Difference, lowerCI, upperCI, Star)%>%
                            dplyr::mutate(across(where(is.numeric), round, 3)))%>%
  tab_header(
    title = md("Table Supp.Methods 2: Path B of mediation analysis after balancing levels of data exclusion across conditions."),
    subtitle = "Original (unbalanced) version is table S6. Effects indicate influence of mediators on network detectability above and beyond independent variables."
  ...
  cols_label(
    ...
    Estimate = md("**Effect estimate**"),
    Difference = md("**Effect Difference (Balanced-original)**"),
    lowerCI = md("**Confidence Interval (lower)**"),
    upperCI = md("**Confidence Interval (upper)**"),
    Star = md("**Significant?**")
  )%>%
...
#format as a table, selecting only the rows of interest
pathcprime_effects_tibble <- read.csv('./mediation_stats_tables/pathcprime_effects_manuallymod.csv') #manually add difference column - it was the easiest way to include it
pathcprime_effects_table <- gt(pathcprime_effects_tibble %>% 
                                 dplyr::select(`indep_var_list2_full..iter_i..`, Estimate, Difference, CI.Lower, CI.Upper, Star)%>%
                                 dplyr::mutate(across(where(is.numeric), round, 3)))%>%
  tab_header(
    title = md("Table Supp.Methods1: Path C' of mediation analysis after balancing levels of data exclusion across conditions."),
    subtitle = "Original (unbalanced) version is table S5. Effects indicate dependence of network detectability on independent variables while controlling for mediators."
  ) %>%
  cols_label(
...
gtsave(pathcprime_effects_table, "./mediation_stats_tables/pathcprime_effects_balanced_table.png")

############################################ indirect effects
#print all the indirect effects in a loop - interaction terms are not shown because cannot be interpreted as mediators                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
df_mediation_estimates <- data.frame(matrix(nrow = 0, ncol=9))
