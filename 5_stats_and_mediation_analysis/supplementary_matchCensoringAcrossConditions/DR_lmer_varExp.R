...
write.csv(df, "./master_DR_and_phgy_variableWindow_sparse_matchNumWindows-over-all.csv")
...
patha8 = lm(Number.of.Timepoints ~  strain*isoflurane_percent + sex*isoflurane_percent + strain*dex_conc + sex*dex_conc + actual_ses_order + Time.after.isoflurane.change, data = df)
summary(patha8)
summary(patha0)
summary(patha1)
summary(patha2)
...
patha_list = list(patha0, patha1, patha2, patha3, patha4, patha5, patha6, patha7, patha8)
med_var_list = list('RR', 'RRV', 'RV', 'HR', 'HRV', 'PVI', 'SpO2', 'Mean FD', 'Number of timepoints per window')
i=1
...
total_path_lm = lm(network_detectability ~  RR..mean.in.window + RRV..mean.in.window + RV..mean.in.window + HR..mean.in.window +PVI..mean.in.window + HRV..mean.in.window + SpO2..mean.in.window + Mean.FD..mean.in.window + strain + isoflurane_percent + sex +dex_conc + actual_ses_order + Time.after.isoflurane.change,
                  data = df_scaled)
print('running relimp total path')
...
relimp_tibble <-tibble(regressors, LMG, first, last) |>
  ...
  add_row(regressors = 'total instantaneous', LMG = 16.4, first = 26.5, last = 8.9) 
...
######################################### variance explained by each regressor ###################
print("Running relimp on phgy var")
...
summ(patha8)
gtsave(parga8, "./mediation_path_effectSize_csvs/patha8.png")